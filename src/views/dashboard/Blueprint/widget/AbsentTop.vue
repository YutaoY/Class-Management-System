...
const startDataChart = () => {
  const result = getStudentViolationRanking()
 ...
 */
const getStudentViolationRanking = (): StudentViolation[] => {
  ...
  tasks.forEach(task => {
    ...
    task.list.check.forEach(record => {
      // 仅统计违纪状态
      if ([
        AttendanceEnum.WD, // 未到
        AttendanceEnum.KK, // 旷课
        AttendanceEnum.ZT  // 早退
